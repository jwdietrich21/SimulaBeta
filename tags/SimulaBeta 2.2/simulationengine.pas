...
type
  ...
  public
    t, P, Q, R, G, S, I, M, N, W: array of extended;
    constructor Create;
    destructor Destroy;
...
const
  ...
  f0 = 0.85;                           // bio-availability of glucose
                                       // [Dalla Man et al. 2004 and Anderwald et al. 2010]
  c0 = 68e-3 / (3e-3 / SecsPerMin);    // Calibration factor gut glucose absorption
                                       // [Brubaker et al 2007]: ca. 3 mmol/min
var
  blocks: TBlocks;
  Q, R, S, M, N, W: extended;
  i: integer;
begin
  W := 0;
  if nmax > 0 then
    ...
    begin
      ...
      if gTestInfo.kind = tkOGTT then
        begin
          ...
          begin
            ...
            W := 75 / MGlucose * f0 / c0 / (p1 + exp(betaGI * t));
          end
        end;
      R := Q + W;
      blocks.G1.input := R;
      ...
      if gTestInfo.kind = tkfsIGT then
        begin
          if i >= gTestInfo.startTime then
          begin
            // Dosage 21 g for 70 kg
            Glc := Glc + (gStrucPars.alphaG * 21 / MGlucose);
            blocks.G1.x1 := Glc;
